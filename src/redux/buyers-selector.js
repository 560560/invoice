export const getBuyersSelector = (state) => {
  ...
  if (state.buyersPage.nameSearchSelector && state.buyersPage.buyers) {
    return [...state.buyersPage.buyers]
      .filter((buyer) =>
        buyer.name.toLowerCase().includes(state.buyersPage.nameSearchSelector)
      )
      .sort((prev, next) => {
        if (state.buyersPage.sortingFlow === "increase")
          return prev[sortingField] - next[sortingField];
        else return next[sortingField] - prev[sortingField];
      });
  } else
    return [...state.buyersPage.buyers].sort((prev, next) => {
      if (state.buyersPage.sortingFlow === "increase")
        return prev[sortingField] - next[sortingField];
      else return next[sortingField] - prev[sortingField];
    });
